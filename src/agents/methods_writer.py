"""Data and Methodology section writer.
...
class MethodsWriterConfig:
    ...
    @classmethod
    def from_context(cls, context: Dict[str, Any]) -> "MethodsWriterConfig":
        raw = context.get("methods_writer")
...
class MethodsWriterAgent(BaseAgent):
    """Deterministic Data and Methodology writer constrained by artifacts."""

    def __init__(self, client=None):
        system_prompt = (
            "You write a Data and Methodology section for an academic paper. "
    ...
    async def execute(self, context: dict) -> AgentResult:
        ...
        cfg = MethodsWriterConfig.from_context(context)
        section_id = str(context.get("section_id") or "methods").strip() or "methods"
        section_title = str(context.get("section_title") or "Data and Methodology").strip() or "Data and Methodology"

        output_path, rel = resolve_section_output_path(pf, section_id=section_id)
        output_path.parent.mkdir(parents=True, exist_ok=True)

        if not cfg.enabled:
            latex = f"\\section{{{_latex_escape(section_title)}}}\n% Methods writer disabled\n"
        ...
        chunks.append("This section describes the data sources and the empirical methodology.")

        if not source_ids:
            chunks.append("% Methods writer downgraded due to missing evidence")
            chunks.append("\\textit{Data source evidence is not yet available; statements are non-definitive.}")
        else:
            chunks.append("\\subsection{Data sources}")
            ...
            for sid in source_ids:
                ...
                if cite_tex:
                    ...
                    chunks.append(f"\\item {safe_sid}. \\textit{{(Citation linkage missing.)}}")
            chunks.append("\\end{itemize}")

        chunks.append("\\subsection{Metrics and computations}")
        if missing_metric_keys:
            chunks.append("% Methods writer downgraded due to missing metrics")
        ...
        if present_metric_keys:
            ...
            chunks.append("Metrics are pending computation.")
