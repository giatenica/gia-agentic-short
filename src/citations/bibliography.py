...
def _overlay_bibliographic_fields(base: Dict[str, Any], preferred: Dict[str, Any]) -> Dict[str, Any]:
    """Overlay bibliographic fields from preferred onto base.

    The citation_key from base is preserved so LaTeX references continue to work.
    """
    ...
    for key in (
        ...
        if key in preferred and preferred.get(key) not in (None, "", [], {}):
            out[key] = preferred.get(key)
    return out


def _index_records_for_version_preference(
...
def _resolve_version_ref(
    ...
    *,
    ...
    lower = value.lower()
    if "/" in value or lower.startswith(("doi:", "http://", "https://")):
        try:
    ...
    return by_key.get(value.lower())


def _apply_version_preference_for_bibtex(
    record: Dict[str, Any],
    *,
    by_key: Dict[str, Dict[str, Any]],
    by_doi: Dict[str, Dict[str, Any]],
    ...
    if vtype not in ("preprint", "working_paper"):
        return record

    target = _resolve_version_ref(version.get("related_published"), by_key=by_key, by_doi=by_doi)
    if not isinstance(target, dict):
        return record

    return _overlay_bibliographic_fields(record, target)
...
def build_bibliography(
    ...
    *,
    ...
    # Write BibTeX.
    by_key, by_doi = _index_records_for_version_preference(deduped_sorted)
    entries = [
        citation_record_to_bibtex(
            _apply_version_preference_for_bibtex(
                r,
                by_key=by_key,
                by_doi=by_doi,
